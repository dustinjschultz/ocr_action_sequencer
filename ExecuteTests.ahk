...
; But once we're in one of the unit tests, located in a subfolder from where the main script it,
...
;
; This means we have two options: Make it so %A_ScriptDir% is the same as the main script's (the option
; that this file's existence give us), or try to %Include% not based on %A_ScriptDir%.
...
; dive too deep here). The one caveat to this is if you put your
; #Include within a function, but even that doesn't help us because (quoting the docs) "A script behaves as
...
; because even if we did load something in that way, it's scope would only be that function, which
...
; one via file manipulation (since that can use dynamic files), place it next to your main script, then load
; your main script with your main script including relative to itself. Maybe I'll do this one day, but
